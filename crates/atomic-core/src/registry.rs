...
/// A configured sync source (full record, including sensitive token).
#[derive(Debug, Clone)]
pub struct SyncSource {
    pub id: String,
    pub name: String,
    /// Source type: "chatgpt", "claude", "markdown_dir", "remote_atomic", "log_file"
    pub source_type: String,
    pub source_url: Option<String>,
    pub source_path: Option<String>,
    /// Raw token for authenticating with remote sources (stored in DB, never returned in API).
    pub source_token: Option<String>,
    /// Target database ID; None means use the active database.
    pub target_db_id: Option<String>,
    /// Polling interval in seconds; 0 means manual-only.
...
/// Public view of a sync source (token is never included).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[cfg_attr(feature = "openapi", derive(utoipa::ToSchema))]
pub struct SyncSourceInfo {
    ...
    pub last_sync_status: Option<String>,
    pub created_at: String,
    /// Whether a source token has been configured (the token itself is never returned).
    pub has_token: bool,
}
...
impl Registry {
    ...
    pub fn create_sync_source(
        ...
        interval_secs: i64,
    ) -> Result<SyncSourceInfo, AtomicCoreError> {
        let conn = self.conn.lock().map_err(|e| AtomicCoreError::Lock(e.to_string()))?;
        let id = Uuid::new_v4().to_string();
        let now = Utc::now().to_rfc3339();
        let has_token = source_token.is_some();

        conn.execute(
            "INSERT INTO sync_sources
             (id, name, source_type, source_url, source_path, source_token, target_db_id, interval_secs, enabled, created_at)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, 1, ?9)",
            rusqlite::params![
                &id, name, source_type, source_url, source_path,
                source_token, target_db_id, interval_secs, &now
            ],
        )?;
        ...
        Ok(SyncSourceInfo {
            ...
            target_db_id: target_db_id.map(String::from),
            interval_secs,
            enabled: true,
            last_synced_at: None,
    ...
    /// List all sync sources (public view, no tokens).
    pub fn list_sync_sources(&self) -> Result<Vec<SyncSourceInfo>, AtomicCoreError> {
        let conn = self.conn.lock().map_err(|e| AtomicCoreError::Lock(e.to_string()))?;
        let mut stmt = conn.prepare(
            "SELECT id, name, source_type, source_url, source_path,
                    target_db_id, interval_secs, enabled,
                    last_synced_at, last_sync_status, created_at,
                    (source_token IS NOT NULL) as has_token
             FROM sync_sources ORDER BY created_at ASC",
        )?;
    ...
    /// Get the full sync source record (includes raw token) for internal scheduler use.
    pub fn get_sync_source_internal(&self, id: &str) -> Result<SyncSource, AtomicCoreError> {
        let conn = self.conn.lock().map_err(|e| AtomicCoreError::Lock(e.to_string()))?;
        conn.query_row(
            "SELECT id, name, source_type, source_url, source_path, source_token,
                    target_db_id, interval_secs, enabled,
                    last_synced_at, last_sync_status, created_at
            ...
            |row| {
                Ok(SyncSource {
                    ...
                    source_url: row.get(3)?,
                    source_path: row.get(4)?,
                    source_token: row.get(5)?,
                    target_db_id: row.get(6)?,
        ...
        .map_err(|_| AtomicCoreError::NotFound(format!("sync source '{id}' not found")))
    }

    /// List all enabled sync sources with their raw tokens for the scheduler.
    pub fn list_sync_sources_internal(&self) -> Result<Vec<SyncSource>, AtomicCoreError> {
        let conn = self.conn.lock().map_err(|e| AtomicCoreError::Lock(e.to_string()))?;
        let mut stmt = conn.prepare(
            "SELECT id, name, source_type, source_url, source_path, source_token,
                    target_db_id, interval_secs, enabled,
                    last_synced_at, last_sync_status, created_at
             FROM sync_sources WHERE enabled = 1 ORDER BY created_at ASC",
        )?;
    ...
    /// Update a sync source's configuration fields.
    ///
    /// Only non-`None` options are updated.
    pub fn update_sync_source(
        ...
        source_token: Option<Option<&str>>,
        target_db_id: Option<Option<&str>>,
        ...
        // Verify it exists
        let exists: bool = conn
            .query_row("SELECT 1 FROM sync_sources WHERE id = ?1", [id], |_| Ok(true))
            .unwrap_or(false);
        if !exists {
            return Err(AtomicCoreError::NotFound(format!("sync source '{id}' not found")));
        }

        if let Some(n) = name {
            conn.execute("UPDATE sync_sources SET name = ?1 WHERE id = ?2", rusqlite::params![n, id])?;
        }
        if let Some(v) = source_url {
            conn.execute("UPDATE sync_sources SET source_url = ?1 WHERE id = ?2", rusqlite::params![v, id])?;
        }
        if let Some(v) = source_path {
            conn.execute("UPDATE sync_sources SET source_path = ?1 WHERE id = ?2", rusqlite::params![v, id])?;
        }
        if let Some(v) = source_token {
            conn.execute("UPDATE sync_sources SET source_token = ?1 WHERE id = ?2", rusqlite::params![v, id])?;
        }
        if let Some(v) = target_db_id {
            conn.execute("UPDATE sync_sources SET target_db_id = ?1 WHERE id = ?2", rusqlite::params![v, id])?;
        }
        if let Some(secs) = interval_secs {
            conn.execute("UPDATE sync_sources SET interval_secs = ?1 WHERE id = ?2", rusqlite::params![secs, id])?;
        }
        if let Some(en) = enabled {
            conn.execute("UPDATE sync_sources SET enabled = ?1 WHERE id = ?2", rusqlite::params![en as i64, id])?;
        }
    ...
    pub fn delete_sync_source(&self, id: &str) -> Result<(), AtomicCoreError> {
        let conn = self.conn.lock().map_err(|e| AtomicCoreError::Lock(e.to_string()))?;
        let rows = conn.execute("DELETE FROM sync_sources WHERE id = ?1", [id])?;
        if rows == 0 {
            return Err(AtomicCoreError::NotFound(format!("sync source '{id}' not found")));
        }
        Ok(())
    ...
    pub fn record_sync_result(
        ...
        let now = Utc::now().to_rfc3339();
        conn.execute(
            "UPDATE sync_sources SET last_synced_at = ?1, last_sync_status = ?2 WHERE id = ?3",
